...
#     all_in_players = []  # オールインプレイヤーリスト
#     last_raiser = None
#     last_raise_amount = big_blind
#     has_raise_right = True  # レイズ権の有無
    
#     # スモールブラインドとビッグブラインドを支払う
#     for name, position in player_positions.items():
#         if position == "SB":
...
#                 return player_bets, player_stacks, pot_size, active_players
            
#             while True:  # 無効な選択時に再入力させるループ
#                 available_actions = ["F"]
...
#                     else:
#                         available_actions.append("A")  # スタック不足時はオールインのみ可
#                 else:
#                     available_actions.append("K")  # ベットが不要ならチェック可
                    
#                 if has_raise_right and player_stacks[player] >= (current_bet + min_raise):
#                     available_actions.append("R")
                
#                 if player_stacks[player] > 0:
#                     available_actions.append("A")  # 常にオールインの選択肢を表示
...
#                     if player_positions[player] == "SB":
#                         current_bet = all_in_amount + small_blind  # SBのオールインにはSB額を追加
#                     elif player_positions[player] == "BB":
#                         current_bet = all_in_amount + big_blind   # BBのオールインにはBB額を追加
#                     else:
#                         current_bet = all_in_amount

#                     if all_in_amount >= current_bet + min_raise:
#                         min_raise = all_in_amount - current_bet
#                         last_raiser = player
#                         has_raise_right = True
#                     else:
#                         has_raise_right = False  # 最小レイズ額未満ならレイズ権なし
                    
#                     print(f"{player} はオールインしました ({all_in_amount})。")
#                     all_called = False
#                 break  # 有効な選択肢が入力されたらループを抜ける
...
#     # ポットの作成
#     pot_list = []
#     sorted_all_in = sorted(all_in_players, key=lambda x: x[1])  # オールイン額が小さい順
...
#         pot_list.append((pot_size, {p for p, _ in sorted_all_in[i:]}))
